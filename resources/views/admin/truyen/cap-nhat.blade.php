    ...
    <script>
        ...
        $('#upload-dropzone').dropzone({
            ...
            init: function () {
                ...
                const mockFile = { name: "demo", size: 12345 };
                this.emit("addedfile", mockFile);
                this.emit("thumbnail", mockFile,'{{asset($truyen->anh_bia)}}');
...
@section('content')
    <div class="d-flex align-items-center mb-3">
        <div>
            ...
            <h1 class="page-header mb-0">Cập nhật truyện</h1>
        </div>
    ...
    <div class="card">
        <div class="tab-content p-4">
            <div class="tab-pane fade show active" id="allTab">
                <form action="{{route('truyen.update',$truyen->ma_truyen)}}" method="post" >
                    @csrf
                    @method('put')
                    <div class="row">
                        ...
                        <div class="col-xl-6">
                            ...
                            <div class="form-group">
                                ...
                                <input value="{{$truyen->ten}}" name="ten" type="text" class="form-control" id="ten" >
                            </div>
                            <div class="form-group">
                                <label for="product-name">Tên tác giả</label>
                                <input value="{{$truyen->tac_gia}}" name="tac_gia" type="text" class="form-control" id="tac_gia" >
                            </div>
                            <div class="form-group">
                                <label for="product-name">Nguồn gốc</label>
                                <input value="{{$truyen->xuat_xu}}" name="xuat_xu" type="text" class="form-control" id="xuat_xu" >
                            </div>
                        </div>
                                        ...
                                        @foreach($theLoai as $k => $tl)
                                            @if($k % 2 === 0)
                                                <div class="custom-control custom-checkbox pl-25px mr-n2">
                                                    <input {{$truyen->ma_the_loai === $tl->ma_the_loai ? 'checked' :''}} value="{{$tl->ma_the_loai}}" type="radio" class="custom-control-input" id="the-loai-{{$tl->ma_the_loai}}" name="ma_the_loai">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        @foreach($danhMuc as $k => $dm)
                                            @if($k % 2 === 0)
                                                <div class="custom-control custom-checkbox pl-25px mr-n2">
                                                    <input {{$truyen->ma_danh_muc === $dm->ma_danh_muc ? 'checked' :''}} value="{{$dm->ma_danh_muc}}" type="radio" class="custom-control-input" id="danh-muc-{{$dm->ma_danh_muc}}" name="ma_danh_muc">
                        ...
                        <div class="col-xl-12">
                            <div class="form-group">
                                <input type="hidden" name="anh_bia" id="anh-bia" value="{{asset($truyen->anh_bia)}}">
...
{{--                                        <input type="file" name="anh_bia" >--}}
                        ...
                        <div class="col-xl-12">
                            <div class="form-group">
                                ...
                                <textarea name="gioi_thieu_truyen" class="form-control" id="product-description" rows="30">{{$truyen->gioi_thieu_truyen}}</textarea>
                            </div>
